...
def generate_response(user_query, situation, service_data):
    if situation['confidence'] < 0.7 or situation['service_id'] == 'unknown':
        return """
    ...
    The user is asking a question in {situation['language_detected']}. 
    ...
    3. If the user mentions rejection ({situation['rejection_mentioned']}), focus entirely on the 'rejection_guidance' from the JSON.
    ...
    *(Verified from: [Source] on [Date])*
    ...
    Note: Do NOT output raw HTML code blocks for the divs, just inject them normally in the text so Streamlit renders them as markdown html.
    """
